...
class CentralizedLoginView(APIView):
    ...
    def post(self, request):
        ...
        from apps.accounts.models import TenantMembership
        from apps.accounts.serializers import CentralizedLoginSerializer, UserSerializer
        ...
        with schema_context("public"):
            memberships = (
                TenantMembership.objects.filter(user=user, status="active")
...
class UserTenantsView(APIView):
    ...
    def get(self, request):
        ...
        with schema_context("public"):
            ...
            memberships = (
                PublicTenantMembership.objects.filter(
                    email_hash=email_hash, status="active"
                )
                .select_related("tenant")

...
def _resolve_work_order(work_order_id: Union[str, int, None]) -> Optional[int]:
    ...
    if isinstance(work_order_id, str):
        try:
            ...
            logger.debug(
                f"work_order_id '{work_order_id}' is not numeric, storing in meta only"
            )
            return None
...
class CostEngineService:
    ...
    def process_work_order_closed(
        ...
        if cost_center_id:
            try:
                ...
                logger.warning(
                    f"CostCenter {cost_center_id} não encontrado, usando default"
                )
        ...
        # Mapear categoria
        finance_category = cls.CATEGORY_MAP.get(
    ...
    def _process_third_party(
        ...
        if total_third_party_cost > 0:
            ...
            if existing:
                logger.info(
                    f"Third party transaction already exists: {idempotency_key}"
                )
                result["skipped"] = 1
                result["transactions"].append(str(existing.id))
            else:

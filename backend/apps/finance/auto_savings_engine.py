...
class AutoSavingsEngine:
            ...
            if baseline.baseline_type == Baseline.BaselineType.ENERGY:
                ...
                if result['success']:
                    ...
                    baseline.save(update_fields=[
                        'before_end', 'before_avg_value', 'before_total_value',
                        'before_days', 'before_data_points', 'updated_at'
                    ])
            else:
                result['error'] = f"Tipo de baseline '{baseline.baseline_type}' não suportado"
                
        except Exception as e:
            logger.exception(f"Erro ao coletar dados 'antes' para baseline {baseline.id}")
            result['error'] = str(e)
            ...
            if baseline.baseline_type == Baseline.BaselineType.ENERGY:
                ...
                if result['success']:
                    ...
                    baseline.save(update_fields=[
                        ...
                        'after_days', 'after_data_points', 'updated_at'
                    ])
            else:
                result['error'] = f"Tipo de baseline '{baseline.baseline_type}' não suportado"
                
        except Exception as e:
            logger.exception(f"Erro ao coletar dados 'depois' para baseline {baseline.id}")
            result['error'] = str(e)
    ...
    def calculate_savings(
        ...
        if baseline.before_days < cls.MIN_DAYS_FOR_BASELINE:
            result['error'] = f"Período 'antes' insuficiente ({baseline.before_days} dias, mínimo {cls.MIN_DAYS_FOR_BASELINE})"
            return result
        
        if baseline.after_days < cls.MIN_DAYS_FOR_BASELINE:
            result['error'] = f"Período 'depois' insuficiente ({baseline.after_days} dias, mínimo {cls.MIN_DAYS_FOR_BASELINE})"
            return result
        ...
        for baseline in collecting_after:
            ...
            try:
                ...
                if res['success']:
                    ...
                    if baseline.after_days >= cls.MIN_DAYS_FOR_BASELINE:
                        calc_res = cls.calculate_savings(baseline, tenant_id=tenant_id, user=user)
                        if calc_res['success']:
                            result['calculated'] += 1
            except Exception as e:
                result['errors'].append({
    ...
    def start_intervention(
        ...
        cls.collect_before_data(baseline, end_date=intervention_date - timedelta(days=1))

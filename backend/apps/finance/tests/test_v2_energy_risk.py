...
class EnergyCostEngineTests(TenantTestCase):
    ...
    def test_process_reading_creates_transaction(self, mock_publish):
        ...
        result = EnergyCostEngine.process_reading(reading, tenant_id='test', user=self.user)
        ...
        self.assertEqual(reading.cost_transaction.transaction_type, CostTransaction.TransactionType.ENERGY)
        self.assertEqual(reading.cost_transaction.category, CostTransaction.Category.ENERGY)
    ...
    def test_process_reading_idempotent(self, mock_publish):
        ...
        result1 = EnergyCostEngine.process_reading(reading, tenant_id='test', user=self.user)
        result2 = EnergyCostEngine.process_reading(reading, tenant_id='test', user=self.user)
        ...
        self.assertEqual(CostTransaction.objects.filter(
...
class RiskSnapshotModelTests(TenantTestCase):
    ...
    def test_create_risk_snapshot(self):
        ...
        self.assertEqual(snapshot.total_impact_cost, Decimal('22000.00'))  # 10000 + 12000
...
class BARAPITests(TenantTestCase):
    ...
    def test_bar_cost_center(self):
        ...
        request = self.factory.get(f'/api/finance/bar/cost-center/{self.cost_center.id}/')
        force_authenticate(request, user=self.user)

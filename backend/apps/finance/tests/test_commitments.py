...
from apps.finance.models import CostCenter, Commitment
...
class CommitmentActionsTests(BaseCommitmentTestCase):
    ...
    def test_approve_commitment(self, mock_publish):
        ...
        request = self.factory.post(f'/api/finance/commitments/{commitment.id}/approve/')
    ...
    def test_reject_commitment(self):
        ...
        self.assertEqual(commitment.rejection_reason, 'Valor excede o limite do orçamento mensal.')
...
class CommitmentSummaryTests(BaseCommitmentTestCase):
    ...
    def test_summary(self):
        ...
        self.assertEqual(
            Decimal(str(response.data['totals']['total_amount'])),
            Decimal('5000.00')
        )
        self.assertEqual(
            Decimal(str(response.data['totals']['approved_amount'])),
            Decimal('3000.00')
        )
        self.assertEqual(
            Decimal(str(response.data['totals']['pending_amount'])),
            Decimal('1500.00')
        )
...
class CommitmentFilterTests(BaseCommitmentTestCase):
    ...
    def test_filter_by_cost_center(self):
        ...
        request = self.factory.get(
            ...
            {'cost_center': str(self.cost_center2.id)}
        )
        force_authenticate(request, user=self.user)
        view = CommitmentViewSet.as_view({'get': 'list'})
        response = view(request)
    ...
    def test_filter_by_category(self):
        ...
        request = self.factory.get('/api/finance/commitments/', {'category': 'preventive'})
        force_authenticate(request, user=self.user)
        view = CommitmentViewSet.as_view({'get': 'list'})
        response = view(request)
    ...
    def test_filter_by_budget_month(self):
        ...
        request = self.factory.get('/api/finance/commitments/', {'budget_month': '2024-06-01'})
        force_authenticate(request, user=self.user)
        view = CommitmentViewSet.as_view({'get': 'list'})
        response = view(request)
    ...
    def test_filter_by_amount_range(self):
        ...
        request = self.factory.get(
            '/api/finance/commitments/',
            {'min_amount': '1500', 'max_amount': '2500'}
        )
        force_authenticate(request, user=self.user)
        view = CommitmentViewSet.as_view({'get': 'list'})
        response = view(request)

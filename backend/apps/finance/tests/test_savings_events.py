...
class SavingsEventAPITests(TenantTestCase):
    ...
    def test_update_savings_event(self):
        ...
        request = self.factory.patch(
            f'/api/finance/savings-events/{event.id}/',
            data=data,
            format='json'
        )
        force_authenticate(request, user=self.user)
    ...
    def test_create_invalid_evidence_format(self):
        ...
        data = {
            ...
            'description': 'Teste',
            'evidence': {
                'invalid_key': 'valor'  # Campo não permitido
            }
        }
    ...
    def test_create_publishes_event(self, mock_publish):
        ...
        self.assertEqual(
            call_args.kwargs['data']['cost_center_id'], 
            str(self.cost_center.id)
        )
    ...
    def test_summary_endpoint(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Decimal(str(response.data['total_savings'])), Decimal('1750.00'))
    ...
    def test_by_cost_center_endpoint(self):
        ...
        self._create_savings_event(
            savings_amount=Decimal('500.00'),
            cost_center=self.cost_center_2
        )
        self._create_savings_event(
            savings_amount=Decimal('300.00'),
            cost_center=self.cost_center
...
class BudgetSummaryAPITests(TenantTestCase):
    ...
    def test_budget_summary_monthly(self):
        ...
        self.assertEqual(Decimal(str(response.data['variance'])), Decimal('1500.00'))  # 5000 - 3500
    ...
    def test_budget_summary_empty_month(self):
        ...
        request = self.factory.get(
            '/api/finance/budget-summary/',
            {'month': '2023-01-01'}  # Mês sem dados
        )
        force_authenticate(request, user=self.user)
    ...
    def test_budget_summary_by_category(self):
        ...
        self.assertEqual(
            Decimal(str(categories['preventive']['planned'])),
            Decimal('5000.00')
        )
    ...
    def test_budget_summary_year(self):
        ...
        request = self.factory.get(
            '/api/finance/budget-summary/year/',
            {'year': '2024'}
        )
        force_authenticate(request, user=self.user)
        ...
        # Verificar junho
        june_data = next(
            m for m in response.data['by_month'] 
            if m['month'] == '2024-06-01'
        )

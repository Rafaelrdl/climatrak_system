...
class EnergyCostEngine:
    ...
    def calculate_reading_cost(
        ...
        if reading.kwh_peak > 0 or reading.kwh_off_peak > 0:
            # Custo ponta
            peak_rate = tariff.rate_peak + cls._get_flag_surcharge(tariff, reading.bandeira)
            peak_cost = reading.kwh_peak * peak_rate
            
            # Custo fora ponta
            off_peak_rate = tariff.rate_off_peak + cls._get_flag_surcharge(tariff, reading.bandeira)
    ...
    def process_reading(
        ...
        # Idempotency key
        idempotency_key = f"energy:{reading.asset_id}:{reading.reading_date.isoformat()}"
        ...
        try:
            ...
            reading.save(update_fields=['calculated_cost', 'cost_transaction', 'updated_at'])

...
class BudgetMonthSerializer(serializers.ModelSerializer):
    ...
    locked_by_name = serializers.CharField(
        source="locked_by.get_full_name", read_only=True, allow_null=True
    )
...
class CostTransactionSerializer(serializers.ModelSerializer):
    ...
    asset_tag = serializers.CharField(
        source="asset.tag", read_only=True, allow_null=True
    )
    work_order_code = serializers.CharField(
        source="work_order.code", read_only=True, allow_null=True
    )
    created_by_name = serializers.CharField(
        source="created_by.get_full_name", read_only=True, allow_null=True
    )
...
class LedgerAdjustmentCreateSerializer(serializers.Serializer):
    ...
    reason = serializers.CharField(
        min_length=10, help_text="Justificativa para o ajuste (mínimo 10 caracteres)"
    )
    ...
    occurred_at = serializers.DateTimeField(
        required=False, help_text="Data da ocorrência (default: agora)"
    )
    description = serializers.CharField(
        required=False, allow_blank=True, help_text="Descrição adicional"
    )
...
class CommitmentCreateSerializer(serializers.ModelSerializer):
    ...
    submit = serializers.BooleanField(
        default=False, write_only=True, help_text="Se True, já submete para aprovação"
    )
...
class CommitmentRejectSerializer(serializers.Serializer):
    ...
    reason = serializers.CharField(
        min_length=10, help_text="Motivo da rejeição (mínimo 10 caracteres)"
    )
...
class SavingsEventSerializer(serializers.ModelSerializer):
    ...
    alert_message = serializers.CharField(
        source="alert.message", read_only=True, allow_null=True
    )
    created_by_name = serializers.SerializerMethodField()
...
class SavingsEventSummarySerializer(serializers.Serializer):
    ...
    by_event_type = serializers.ListField(
        child=serializers.DictField(), help_text="Economias agrupadas por tipo"
    )
...
class BudgetMonthlySummarySerializer(serializers.Serializer):
    ...
    variance = serializers.DecimalField(
        max_digits=15, decimal_places=2, help_text="planned - actual"
    )
    variance_percent = serializers.DecimalField(
        max_digits=5, decimal_places=2, help_text="(planned - actual) / planned * 100"
    )

    # Breakdown por categoria
    by_category = serializers.ListField(
        child=serializers.DictField(), help_text="Valores agrupados por categoria"
    )
...
class EnergyReadingSerializer(serializers.ModelSerializer):
    ...
    tariff_name = serializers.CharField(
        source="tariff.name", read_only=True, allow_null=True
    )
...
class BaselineSerializer(serializers.ModelSerializer):
    ...
    def get_progress_percent(self, obj):
        ...
        if obj.status == Baseline.Status.COLLECTING_BEFORE:
            ...
            after_progress = (
                min(50, (obj.after_days / min_days) * 50) if min_days > 0 else 0
            )
...
class BaselineTransitionSerializer(serializers.Serializer):
    ...
    intervention_date = serializers.DateField(
        required=False, help_text="Data da intervenção (para start_intervention)"
    )
    after_start_date = serializers.DateField(
        required=False, help_text="Data de início do período depois (para start_after)"
    )
...
class BARSummarySerializer(serializers.Serializer):
    ...
    by_cost_center = serializers.ListField(
        child=serializers.DictField(), help_text="BAR agrupado por centro de custo"
    )
    top_risks = serializers.ListField(
        child=serializers.DictField(), help_text="Top riscos ordenados por score"
    )
...
class BARForecastSerializer(serializers.Serializer):
    ...
    forecast = serializers.ListField(
        child=serializers.DictField(), help_text="Projeção mensal de BAR"
    )

...
from .models import CostCenter, RiskSnapshot
...
class BARCalculator:
    ...
    def calculate_bar_for_cost_center(
        ...
        result["avg_risk_score"] = Decimal(str(aggregation["avg_score"] or 0)).quantize(
            Decimal("0.01")
        )
    ...
    def calculate_bar_summary(
        ...
        if snapshot_date:
            ...
            target_date = RiskSnapshot.objects.aggregate(max_date=Max("snapshot_date"))[
                "max_date"
            ]
        ...
        # Top N riscos
        top_risks = snapshots.select_related("asset", "cost_center").order_by(
            "-risk_score"
        )[:top_n]
        result["top_risks"] = [
            {
    ...
    def _calculate_trend(cls, current_date: date) -> Optional[Dict[str, Any]]:
        ...
        dates = (
            RiskSnapshot.objects.filter(snapshot_date__lte=current_date)
        ...
        bar_by_date = []
        for d in dates:
            total = RiskSnapshot.objects.filter(snapshot_date=d).aggregate(
                total=Sum("risk_score")
            )["total"] or Decimal("0")
            bar_by_date.append(
                {
                    "date": d.isoformat(),

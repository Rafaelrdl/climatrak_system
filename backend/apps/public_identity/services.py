...
class TenantAuthService:
        ...
        try:
            with schema_context(tenant.schema_name):
                ...
                try:
                    ...
                    logger.debug(f"User {email} not found in tenant {tenant.schema_name}")
                    return None
                ...
                if not authenticated_user:
                    logger.debug(f"Password mismatch for {email} in tenant {tenant.schema_name}")
                    return None
                
                if not authenticated_user.is_active:
                    logger.debug(f"User {email} is inactive in tenant {tenant.schema_name}")
                    return None
                ...
                return AuthenticatedTenant(
                    ...
                    user_full_name=getattr(authenticated_user, 'full_name', '') or authenticated_user.email,
                    user_avatar=getattr(authenticated_user, 'avatar', None),
                    user_is_staff=authenticated_user.is_staff,
                )
                
        except Exception as e:
            logger.exception(f"Error authenticating in tenant {tenant.schema_name}: {e}")
            return None
    ...
    def _get_user_role(cls, email: str, tenant) -> str:
        ...
        try:
            ...
            logger.exception(f"Error getting role for {email} in tenant {tenant.id}: {e}")
    ...
    def login(cls, email: str, password: str) -> LoginResult:
        ...
        if not candidates:
            logger.info(f"Login attempt for unknown email hash")
            return LoginResult(
                success=False,
        ...
        if not authenticated_tenants:
            logger.info(f"Login failed: password mismatch in all candidate tenants")
            return LoginResult(
                success=False,
                error="Credenciais inválidas."
            )
        ...
        access_token, refresh_token = cls._generate_tokens(
            first_auth.schema_name,
            first_auth.user_id,
            first_auth.user_email
        )
        
        logger.info(f"Login successful for {email} in {len(authenticated_tenants)} tenant(s)")

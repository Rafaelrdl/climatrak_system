...
class Command(BaseCommand):
    ...
    def _clear_index_tables(self, tenant_schema: Optional[str] = None):
        ...
        from apps.public_identity.models import TenantUserIndex, TenantMembership
        ...
        with schema_context('public'):
            if tenant_schema:
                ...
                try:
                    ...
                    count_idx = TenantUserIndex.objects.filter(tenant=tenant).delete()[0]
                    count_mem = TenantMembership.objects.filter(tenant=tenant).delete()[0]
                except Tenant.DoesNotExist:
        ...
        with schema_context(tenant.schema_name):
            ...
            for idx, user in enumerate(users, 1):
                ...
                with schema_context('public'):
                    ...
                    # TenantMembership
                    membership_obj, membership_created = TenantMembership.objects.update_or_create(
                        email_hash=email_hash,
                        tenant=tenant,
                        defaults={
                            'tenant_user_id': user.id,
                            'email_hint': TenantUserIndex.get_email_hint(email),
                            'display_name': getattr(user, 'full_name', '') or email.split('@')[0],
                            'role': role,

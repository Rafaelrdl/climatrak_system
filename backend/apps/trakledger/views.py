...
class BudgetPlanViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request, pk=None):
        ...
        by_category = (
            plan.envelopes.values("category")
            .annotate(total=Sum("amount"))
            .order_by("category")
        )

        by_cost_center = (
            plan.envelopes.values("cost_center__code", "cost_center__name")
            .annotate(total=Sum("amount"))
...
class BudgetMonthViewSet(viewsets.ModelViewSet):
    ...
    def lock(self, request, pk=None):
        ...
        if month.is_locked:
            return Response(
                {"error": "Mês já está bloqueado"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def unlock(self, request, pk=None):
        ...
        if not month.is_locked:
            return Response(
                {"error": "Mês não está bloqueado"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CostTransactionViewSet(viewsets.ModelViewSet):
                ...
                "by_category": [
                    {
                        ...
                        "category_display": dict(CostTransaction.Category.choices).get(
                ...
                "by_type": [
                    {
                        ...
                        "type_display": dict(
                            CostTransaction.TransactionType.choices
                        ).get(item["transaction_type"], item["transaction_type"]),
                        "total_amount": item["total_amount"] or Decimal("0.00"),
                        "transaction_count": item["transaction_count"],
                    }
                    for item in by_type
    ...
    def by_cost_center(self, request):
        ...
        by_cc = (
            queryset.values("cost_center__id", "cost_center__code", "cost_center__name")
            .annotate(total_amount=Sum("amount"), transaction_count=Count("id"))
            .order_by("cost_center__code")
        )
    ...
    def bulk_lock(self, request):
        ...
        queryset.update(is_locked=True, locked_at=now, locked_by=request.user)

        return Response(
            {"message": f"{count} transações bloqueadas", "locked_count": count}
        )
...
class LedgerAdjustmentViewSet(viewsets.ModelViewSet):
    ...
    queryset = LedgerAdjustment.objects.select_related(
        "original_transaction", "adjustment_transaction", "created_by", "approved_by"
    )
    permission_classes = [IsAuthenticated]
    filter_backends = [
        DjangoFilterBackend,
    ...
    def create(self, request, *args, **kwargs):
        ...
        read_serializer = LedgerAdjustmentSerializer(
    ...
    def summary(self, request):
        ...
        by_type = (
            queryset.values("adjustment_type")
            .annotate(
                total_amount=Sum("adjustment_amount"), adjustment_count=Count("id")
            )
                ...
                "by_type": [
                    {
                        ...
                        "type_display": dict(
                            LedgerAdjustment.AdjustmentType.choices
                        ).get(item["adjustment_type"], item["adjustment_type"]),
                        "total_amount": item["total_amount"] or Decimal("0.00"),
...
class CommitmentViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if commitment.status != Commitment.Status.DRAFT:
            ...
            raise PermissionDenied(
                "Apenas compromissos em rascunho podem ser alterados."
            )
        serializer.save()
    ...
    def perform_destroy(self, instance):
        ...
        if instance.status != Commitment.Status.DRAFT:
            ...
            raise PermissionDenied(
                "Apenas compromissos em rascunho podem ser excluídos."
            )
        instance.delete()
    ...
    def approve(self, request, pk=None):
        ...
        serializer = CommitmentSerializer(commitment, context={"request": request})
        return Response(
            {**serializer.data, "event_id": str(event.id) if event else None}
        )
    ...
    def reject(self, request, pk=None):
        ...
        try:
            commitment.reject(
                user=request.user, reason=serializer.validated_data["reason"]
            )
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)

        response_serializer = CommitmentSerializer(
            commitment, context={"request": request}
        )
    ...
    def summary(self, request):
        ...
        approved_total = queryset.filter(status=Commitment.Status.APPROVED).aggregate(
            total=Sum("amount")
        )["total"] or Decimal("0.00")

        pending_total = queryset.filter(status=Commitment.Status.SUBMITTED).aggregate(
            total=Sum("amount")
        )["total"] or Decimal("0.00")
...
from .serializers import SavingsEventCreateSerializer, SavingsEventSerializer
...
class SavingsEventViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        response_serializer = SavingsEventSerializer(
    ...
    def summary(self, request):
        ...
        # Total geral
        totals = queryset.aggregate(
            total_savings=Sum("savings_amount"), count=Count("id")
        )

        # Por tipo de evento
        by_event_type = (
            queryset.values("event_type")
            .annotate(total=Sum("savings_amount"), count=Count("id"))
            .order_by("-total")
        )

        # Por nível de confiança
        by_confidence = (
            queryset.values("confidence")
            .annotate(total=Sum("savings_amount"), count=Count("id"))
            .order_by("confidence")
        )
        ...
        dates = queryset.aggregate(
            start=db_models.Min("occurred_at"), end=db_models.Max("occurred_at")
        )
    ...
    def by_type(self, request):
        ...
        by_type = (
            queryset.values("event_type")
            .annotate(total_savings=Sum("savings_amount"), count=Count("id"))
    ...
    def by_cost_center(self, request):
        ...
        by_cc = (
            queryset.values("cost_center", "cost_center__code", "cost_center__name")
            .annotate(total_savings=Sum("savings_amount"), count=Count("id"))
            .order_by("-total_savings")
        )
...
class BudgetSummaryViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        planned = planned_qs.aggregate(total=Sum("planned_amount"))["total"] or Decimal(
            "0.00"
        )
        ...
        actual_qs = CostTransaction.objects.filter(
            occurred_at__date__gte=month_start, occurred_at__date__lte=month_end
        )
        if cost_center_id:
            actual_qs = actual_qs.filter(cost_center_id=cost_center_id)

        actual = actual_qs.aggregate(total=Sum("amount"))["total"] or Decimal("0.00")
        ...
        savings = savings_qs.aggregate(total=Sum("savings_amount"))["total"] or Decimal(
            "0.00"
        )
    ...
    def _get_category_breakdown(
        self, month_date, month_start, month_end, cost_center_id
    ):
        ...
        for cat_key, cat_display in categories.items():
            ...
            planned_qs = BudgetMonth.objects.filter(
                month=month_date, envelope__category=cat_key
            )
            if cost_center_id:
                planned_qs = planned_qs.filter(envelope__cost_center_id=cost_center_id)
            cat_planned = planned_qs.aggregate(total=Sum("planned_amount"))[
                "total"
            ] or Decimal("0.00")
            ...
            committed_qs = Commitment.objects.filter(
                budget_month=month_date, category=cat_key, status__in=committed_statuses
            )
            if cost_center_id:
                committed_qs = committed_qs.filter(cost_center_id=cost_center_id)
            cat_committed = committed_qs.aggregate(total=Sum("amount"))[
                "total"
            ] or Decimal("0.00")
            ...
            if cost_center_id:
                actual_qs = actual_qs.filter(cost_center_id=cost_center_id)
            cat_actual = actual_qs.aggregate(total=Sum("amount"))["total"] or Decimal(
                "0.00"
            )
    ...
    def year(self, request):
        ...
        # Planned
        planned_qs = BudgetMonth.objects.filter(
            month__gte=year_start, month__lte=year_end
        )
        if cost_center_id:
            planned_qs = planned_qs.filter(envelope__cost_center_id=cost_center_id)
        planned = planned_qs.aggregate(total=Sum("planned_amount"))["total"] or Decimal(
        ...
        # Actual
        actual_qs = CostTransaction.objects.filter(
            occurred_at__date__gte=year_start, occurred_at__date__lte=year_end
        )
        if cost_center_id:
            actual_qs = actual_qs.filter(cost_center_id=cost_center_id)
        actual = actual_qs.aggregate(total=Sum("amount"))["total"] or Decimal("0.00")
    ...
    def _get_monthly_breakdown(self, year, cost_center_id):
        ...
        for month_num in range(1, 13):
            ...
            if cost_center_id:
                planned_qs = planned_qs.filter(envelope__cost_center_id=cost_center_id)
            planned = planned_qs.aggregate(total=Sum("planned_amount"))[
                "total"
            ] or Decimal("0.00")
            ...
            # Actual
            actual_qs = CostTransaction.objects.filter(
                occurred_at__date__gte=month_date, occurred_at__date__lte=month_end
            )
            if cost_center_id:
                actual_qs = actual_qs.filter(cost_center_id=cost_center_id)
            actual = actual_qs.aggregate(total=Sum("amount"))["total"] or Decimal(
                "0.00"
            )
            ...
            if cost_center_id:
                savings_qs = savings_qs.filter(cost_center_id=cost_center_id)
            savings = savings_qs.aggregate(total=Sum("savings_amount"))[
                "total"
            ] or Decimal("0.00")
...
class EnergyReadingViewSet(viewsets.ModelViewSet):
    ...
    def process(self, request, pk=None):
        ...
        if reading.cost_transaction:
            return Response(
                {
                    "error": "Leitura já processada",
                    "transaction_id": str(reading.cost_transaction_id),
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class BaselineViewSet(viewsets.ModelViewSet):
    ...
    def transition(self, request, pk=None):
        ...
        return Response(
            {"error": "Ação desconhecida"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class BARViewSet(viewsets.ViewSet):
    ...
    def summary(self, request):
        ...
        snapshot_date = (
            date.fromisoformat(snapshot_date_str) if snapshot_date_str else None
        )
    ...
    @action(
        detail=False, methods=["get"], url_path="cost-center/(?P<cost_center_id>[^/.]+)"
    )

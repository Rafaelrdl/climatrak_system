...
class TelemetryFilterForm(forms.Form):
    ...
    tenant_slug = forms.SlugField(
        required=True, max_length=100, help_text="Tenant slug (required)"
    )

    device_id = forms.CharField(
        required=False, max_length=255, help_text="Filter by device ID (optional)"
    )

    sensor_id = forms.CharField(
        required=False, max_length=255, help_text="Filter by sensor ID (optional)"
    )
    ...
    offset = forms.IntegerField(
        required=False, min_value=0, initial=0, help_text="Pagination offset"
    )

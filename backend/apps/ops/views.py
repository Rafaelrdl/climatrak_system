...
def telemetry_export_csv(request):
    ...
    writer.writerow(
        ["bucket", "device_id", "sensor_id", "avg", "min", "max", "last", "count"]
    )
...
def chart_data_api(request):
    ...
    if not (tenant_slug and sensor_ids):
        return JsonResponse(
            {"error": "tenant_slug and sensor_ids are required"}, status=400
        )

    # Limit to 10 sensors max for performance
    if len(sensor_ids) > 10:
        return JsonResponse({"error": "Maximum 10 sensors allowed"}, status=400)
...
def export_request(request):
    ...
    if from_timestamp:
        try:
            from_ts = timezone.datetime.fromisoformat(
...
def export_download(request, job_id):
    ...
    if job.status != ExportJob.STATUS_COMPLETED:
        messages.error(
            request,
            f"Export ainda não está pronto (status: {job.get_status_display()})",
        )
        return redirect("ops:export_list")
...
def export_cancel(request, job_id):
    ...
    if job.status not in [ExportJob.STATUS_PENDING, ExportJob.STATUS_PROCESSING]:
        messages.error(
            request,
            f"Não é possível cancelar export com status: {job.get_status_display()}",
        )
        return redirect("ops:export_list")

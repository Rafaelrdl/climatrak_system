...
class AssetType(models.Model):
    ...
    name = models.CharField(
        "Nome", max_length=100, help_text="Nome exibível do tipo de ativo"
    )
    description = models.TextField(
        "Descrição", blank=True, help_text="Descrição opcional do tipo de ativo"
    )
...
class Site(models.Model):
    ...
    address = models.TextField(
        "Endereço", blank=True, help_text="Endereço completo do site"
    )
...
class Sensor(models.Model):
    ...
    def update_last_reading(self, value, timestamp=None):
        ...
        self.is_online = True
        self.save(
            update_fields=["last_value", "last_reading_at", "is_online", "updated_at"]
        )

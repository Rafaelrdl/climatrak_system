...
class AssetSerializer(serializers.ModelSerializer):
    ...
    def validate_tag(self, value):
        ...
        if value:
            ...
            if queryset.exists():
                raise serializers.ValidationError(f"Asset com tag '{value}' já existe.")
        return value
...
class AssetCompleteSerializer(serializers.ModelSerializer):
    ...
    sector_id = serializers.IntegerField(
        source="sector.id", read_only=True, allow_null=True
    )
    sector_name = serializers.CharField(
        source="sector.name", read_only=True, allow_null=True
    )
    subsection_id = serializers.IntegerField(
        source="subsection.id", read_only=True, allow_null=True
    )
    subsection_name = serializers.CharField(
        source="subsection.name", read_only=True, allow_null=True
    )
    ...
    def get_online_sensor_count(self, obj):
        ...
        return Sensor.objects.filter(
            device__asset=obj, is_online=True, is_active=True
        ).count()
    ...
    def get_latest_readings(self, obj):
        ...
        for sensor in sensors:
            ...
            if last_reading:
                metric_key = (
                    sensor.metric_type.lower()
                    if sensor.metric_type
                    else f"sensor_{sensor.id}"
                )
                readings[metric_key] = {
                    "sensor_id": sensor.id,
...
class SensorListSerializer(serializers.ModelSerializer):
    ...
    device_mqtt_client_id = serializers.CharField(
        source="device.mqtt_client_id", read_only=True
    )
    ...
    def validate(self, data):
        ...
        if "device" in data and "metric_type" in data:
            queryset = Sensor.objects.filter(
                device=data["device"], metric_type=data["metric_type"]
            )
            if self.instance:
                queryset = queryset.exclude(pk=self.instance.pk)
            if queryset.exists():

...
from apps.trakservice.services import FinanceLockError, QuoteFinanceService
...
class QuoteFinanceTransactionTests(BaseFinanceBridgeTestCase):
    ...
    def test_transaction_category_from_work_order(self):
        ...
        tx = CostTransaction.objects.get(idempotency_key=f"quote:{quote.id}:service")
...
class QuoteFinanceLockTests(BaseFinanceBridgeTestCase):
    ...
    def test_locked_month_raises_error(self):
        ...
        from apps.trakledger.models import BudgetEnvelope, BudgetMonth, BudgetPlan

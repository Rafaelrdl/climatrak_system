...
class ServiceAssignmentViewSet(viewsets.ModelViewSet):
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="by-technician/(?P<technician_id>[^/.]+)",
    )
...
class TechnicianLocationView(APIView):
    ...
    def get(self, request, technician_id):
        ...
        try:
            ...
            return Response(
                {"detail": "Técnico não encontrado."}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def _get_latest(self, technician):
        ...
        latest_ping = (
            LocationPing.objects.filter(technician=technician)
...
class DailyRouteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return (
            DailyRoute.objects.select_related(
                "technician",
                "technician__user",
                "created_by",
            )
    ...
    def complete_route(self, request, pk=None):
        ...
        if route.status != DailyRoute.Status.IN_PROGRESS:
            return Response(
                {"detail": "Só é possível completar uma rota em progresso."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def update_stop(self, request, pk=None, stop_id=None):
        ...
        try:
            ...
            return Response(
                {"detail": "Parada não encontrada."}, status=status.HTTP_404_NOT_FOUND
            )
...
class QuoteFilter(filters.FilterSet):
    ...
    work_order_number = filters.CharFilter(
        field_name="work_order__number", lookup_expr="icontains"
    )
...
class QuoteViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        """
        from .services import FinanceLockError, QuoteFinanceService
    ...
    @action(
        detail=False,
        methods=["get"],
        url_path="by-work-order/(?P<work_order_id>[^/.]+)",
    )

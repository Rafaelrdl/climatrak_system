...
class DispatchService(TrakServiceBaseService):
    ...
    def get_technician_schedule(cls, technician_id, date_from, date_to):
        ...
        return (
            ...
            .exclude(status=ServiceAssignment.Status.CANCELED)
    ...
    def get_daily_summary(cls, date):
        ...
        stats = (
            ServiceAssignment.objects.filter(scheduled_date=date)
            .values("status")
...
class RoutingService(TrakServiceBaseService):
    ...
    def find_nearest_technician(
        ...
        from .models import LocationPing, TechnicianProfile
        ...
        latest_pings = (
            LocationPing.objects.filter(technician_id__in=technician_ids)
            .values("technician_id")
            .annotate(latest_at=Max("recorded_at"))
        )

        # Build lookup
        latest_map = {p["technician_id"]: p["latest_at"] for p in latest_pings}
    ...
    def calculate_km_summary(cls, technician, date):
        ...
        for ping in pings:
            ...
            if prev_ping:
                distance = cls.haversine_distance(
                    float(prev_ping.latitude),
                    float(prev_ping.longitude),
                    float(ping.latitude),
                    float(ping.longitude),
                )
                km_actual += distance
...
class QuoteFinanceService(TrakServiceBaseService):
    ...
    def process_quote_approved(
        ...
        if quote.status != Quote.Status.APPROVED:
            raise ValueError(
                "Quote must be in APPROVED status to create finance entries"
            )
    ...
    def _create_service_transaction(cls, quote, work_order) -> Dict[str, Any]:
        ...
        if not cost_center:
            logger.warning(
                f"No cost center for quote {quote.id}, skipping service transaction"
            )
            return result
    ...
    def _create_material_transaction(cls, quote, work_order) -> Dict[str, Any]:
        ...
        if not cost_center:
            logger.warning(
                f"No cost center for quote {quote.id}, skipping material transaction"
            )
            return result
    ...
    def _check_month_lock(cls, occurred_at):
        ...
        month_start = (
            occurred_at.replace(day=1).date()
            if hasattr(occurred_at, "date")
            else occurred_at.replace(day=1)
        )
    ...
    def _publish_quote_approved_event(cls, quote, approved_by=None):
        ...
        logger.info(
            f"Published trakservice.quote.approved.v1 event for quote {quote.id}"
        )

...
class TenantAdmin(TenantAdminMixin, admin.ModelAdmin):
    ...
    def tenant_invite_user_view(self, request, tenant_id):
        ...
        from apps.accounts.models import TenantMembership, Invite, User
        from django import forms
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if existing_membership:
                    messages.error(
                        request, 
                        f"⚠️ O usuário {email} já é membro deste tenant."
                    )
                else:
    ...
    def tenant_user_remove_view(self, request, tenant_id, membership_id):
        ...
        if membership.role == 'owner':
            ...
            if owner_count <= 1:
                messages.error(
                    request, 
                    "⚠️ Não é possível remover o último owner do tenant!"
                )
                return redirect('admin:tenant_users', tenant_id=tenant_id)
        ...
        if request.method == 'POST' and 'confirm' in request.POST:
            ...
            messages.success(
                request, 
                f"🗑️ Membro {user_email} removido do tenant {tenant.name}."
            )
            return redirect('admin:tenant_users', tenant_id=tenant_id)
    ...
    def tenant_invite_cancel_view(self, request, tenant_id, invite_id):
        ...
        invite = get_object_or_404(Invite, pk=invite_id, tenant=tenant, status='pending')
        ...
        if request.method == 'POST':
            ...
            messages.success(
                request, 
                f"❌ Convite para {invite.email} cancelado."
            )

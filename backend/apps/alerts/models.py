...
class RuleParameter(models.Model):
    ...
    variable_key = models.CharField(
        max_length=100, blank=True, verbose_name="Chave da Variável"
    )

    # Condição
    operator = models.CharField(
        max_length=10, choices=OPERATOR_CHOICES, verbose_name="Operador"
    )
...
class Rule(models.Model):
    ...
    unit = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="Unidade (deprecated)"
    )
    duration = models.IntegerField(
        default=5, blank=True, null=True, verbose_name="Duração (deprecated)"
    )
    severity = models.CharField(
        max_length=20,
        choices=SEVERITY_CHOICES,
...
class Alert(models.Model):
    ...
    acknowledged_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Reconhecido em"
    )
    ...
    def __str__(self):
        status = (
            "Resolvido"
            if self.resolved
            else ("Reconhecido" if self.acknowledged else "Ativo")
        )

...
from apps.accounts.permissions import CanWrite, IsTenantMember

from .models import Alert, NotificationPreference, Rule
...
class RuleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Rule.objects.select_related("equipment", "created_by").all()
        ...
        if severity:
            ...
            )  # 🔧 Case-insensitive (aceita CRITICAL ou Critical)
    ...
    @action(
        detail=True,
        methods=["post"],
        permission_classes=[IsAuthenticated, IsTenantMember, CanWrite],
    )
...
class AlertViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Alert.objects.select_related(
            "rule", "rule__equipment", "acknowledged_by", "resolved_by"
        ).all()
    ...
    def acknowledge(self, request, pk=None):
        ...
        if notes:
            alert.notes = (
                f"{alert.notes}\n[Reconhecimento] {notes}"
                if alert.notes
                else f"[Reconhecimento] {notes}"
            )

        alert.save()

        response_serializer = self.get_serializer(alert)
        return Response({"status": "acknowledged", "alert": response_serializer.data})
    ...
    def resolve(self, request, pk=None):
        ...
        if alert.resolved:
            return Response(
                {"error": "Alerta já foi resolvido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if notes:
            alert.notes = (
                f"{alert.notes}\n[Resolução] {notes}"
                if alert.notes
                else f"[Resolução] {notes}"
            )

        alert.save()

        response_serializer = self.get_serializer(alert)
        return Response({"status": "resolved", "alert": response_serializer.data})

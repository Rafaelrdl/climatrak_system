...
class JWTCookieAuthentication(JWTAuthentication):
    ...
    def get_user(self, validated_token):
        ...
        token_schema = getattr(self, "_token_schema", None) or validated_token.get(
            "tenant_schema"
        )
        request_schema = getattr(self, "_request_schema", None) or getattr(
            connection, "schema_name", public_schema
        )

        if token_schema:
            if request_schema != public_schema and request_schema != token_schema:
                raise AuthenticationFailed(
                    "Token tenant mismatch", code="tenant_mismatch"
                )
            schema_to_use = token_schema
        else:
            if request_schema != public_schema:
                raise AuthenticationFailed(
                    "Token missing tenant schema", code="tenant_missing"
                )

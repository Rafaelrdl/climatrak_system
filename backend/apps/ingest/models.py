...
class Telemetry(models.Model):
    ...
    device_id = models.CharField(
        max_length=255, db_index=True, help_text="MQTT client ID (device identifier)"
    )
    ...
    created_at = models.DateTimeField(
        default=timezone.now, help_text="When this record was created in the database"
    )
...
class Reading(models.Model):
    ...
    device_id = models.CharField(
        max_length=255, db_index=True, help_text="Device identifier (from MQTT client)"
    )
    ...
    labels = models.JSONField(
        default=dict, blank=True, help_text="Optional metadata (location, unit, etc.)"
    )
    ...
    # Internal timestamp
    created_at = models.DateTimeField(
        default=timezone.now, help_text="When record was inserted into database"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["tenant", "asset_tag", "ts"], name="reading_tenant_asset_ts_idx"
            ),
            models.Index(
                fields=["site", "asset_tag", "ts"], name="reading_site_asset_ts_idx"
            ),
        ]
        constraints = [

...
class KhompSenMLParser(PayloadParser):
        ...
        if isinstance(payload, dict) and "payload" in payload:
            ...
            if isinstance(inner_payload, str):
                try:
                    ...
                    logger.info(
                        f"✅ Payload string convertido para JSON com {len(inner_payload)} elementos"
                    )
        ...
        if base_time:
            try:
                ...
                timestamp = datetime.datetime.fromtimestamp(
                    base_time, tz=datetime.timezone.utc
                )
            except (ValueError, TypeError) as e:
                logger.warning(
                    f"Erro ao converter base_time: {e}, usando timestamp atual"
                )
    ...
    def _auto_register_device(
        self, device_id: str, gateway_id: Optional[str], model: Optional[str]
    ) -> None:
        """
    ...
    def _auto_register_sensor(
        self, device_id: str, sensor_reading: Dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if not device:
                logger.warning(
                    f"⚠️ Dispositivo {device_id} não encontrado para auto-registro do sensor {sensor_id}"
                )
                return
            ...
            logger.error(
                f"❌ Erro ao auto-registrar sensor {sensor_reading.get('sensor_id')}: {e}"
            )

    def _process_sensor_element(
        self, element: Dict[str, Any], base_name: str
    ...
    def _create_sensor_reading(
        ...
        reading = {
            "sensor_id": sensor_id,
            "value": numeric_value,
            "labels": {"name": name, "type": sensor_type, "value_type": value_type},
        }

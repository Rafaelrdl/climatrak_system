...
class StandardParser(PayloadParser):
    ...
    def parse(self, payload: Dict[str, Any], topic: str) -> Dict[str, Any]:
        ...
        if not device_id:
            device_id = client_id
            logger.warning(
                f"device_id não encontrado no payload, usando client_id: {client_id}"
            )
        ...
        for sensor_data in inner_payload.get("sensors", []):
            ...
            sensors.append(
                {"sensor_id": sensor_id, "value": float(value), "labels": labels}
            )
        ...
        result = {
            ...
            "timestamp": timestamp,
            "sensors": sensors,
            "metadata": {"format": "standard", "client_id": client_id},
        }

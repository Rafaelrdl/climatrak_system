            ...
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if (
                        ...
                        and issubclass(attr, PayloadParser)
                        and attr is not PayloadParser
                    ):
                ...
                logger.warning(
                    f"⚠️ Não foi possível importar o módulo de parser {module_path}: {e}"
                )
            except Exception as e:
                logger.error(
                    f"❌ Erro ao carregar parser de {module_path}: {e}", exc_info=True
                )

    def get_parser(
        self, payload: Dict[str, Any], topic: str
    ) -> Optional[PayloadParser]:
        """

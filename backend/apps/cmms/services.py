...
class WorkOrderService:
    ...
    def _get_parts_entries(cls, work_order) -> List[Dict[str, Any]]:
        ...
        for part_usage in work_order.part_usages.all():
            entry = {
                ...
                'part_id': str(part_usage.inventory_item_id) if part_usage.inventory_item_id else None,
                ...
                'unit_cost': float(part_usage.unit_cost) if part_usage.unit_cost else None,
            }
    ...
    def _get_third_party_entries(cls, work_order) -> List[Dict[str, Any]]:
        ...
        for external_cost in work_order.external_costs.all():
            entry = {
                ...
                'invoice_date': str(external_cost.invoice_date) if external_cost.invoice_date else None,
            }
            entries.append(entry)
    ...
    def get_work_order_cost_summary(cls, work_order) -> Dict[str, Any]:
        ...
        """
        # Labor
        labor_data = work_order.time_entries.aggregate(
        ...
        # Third party
        third_party_data = work_order.external_costs.aggregate(
            total=Sum('amount')
        )

    ...
    def test_telemetry_api() -> None:
        ...
        for tenant in tenants:
            with schema_context(tenant.schema_name):
                ...
                print(f"ÐY\"­ Testando API de telemetria para device: {device_id}")
                print("   Endpoint: GET /api/telemetry/history/{device_id}/\n")

                # Criar request simulado
                factory = RequestFactory()

                # Teste 1: Çsltimas 24 horas (sem parÇ½metros)
                print("ÐYõ¦ TESTE 1: Çsltimas 24 horas (padrÇœo)")
                print("-" * 80)
                ...
                if response.status_code == 200:
                    ...
                    if data.get("count", 0) > 0:
                        print("\n   ÐY\"S Amostra dos dados (primeiros 3):")
                        for i, item in enumerate(data.get("data", [])[:3]):
                            print(f"      [{i + 1}] {item}")
                    else:
                        print("\n   ƒsÿ‹÷?  NENHUM DADO RETORNADO! (count=0)")
                        print(
                            "   ƒsÿ‹÷?  Mas sabemos que hÇ­ 64 "
                            "readings nas Ç§ltimas 24h no banco!"
                        )

                    # Mostrar estrutura completa da resposta
                    print("\n   ÐY\"< Estrutura completa da resposta:")
                    print(f"   {json.dumps(dict(response.data), indent=2, default=str)}")

                else:
                    print(f"   ƒ?O Erro: {response.status_code}")
                    print(f"   Response: {response.data}")

                # Teste 2: Com parÇ½metros explÇðcitos (Ç§ltimas 24h)
                print(f"\n{'=' * 80}")
                print("ÐYõ¦ TESTE 2: Com parÇ½metros from/to explÇðcitos")
                print("-" * 80)
                ...
                if response.status_code == 200:
                    ...
                    if data.get("count", 0) > 0:
                        ...
                        print("\n   ƒ?O NENHUM DADO RETORNADO!")

                # Teste 3: Buscar dados RAW (sem agregaÇõÇœo)
                print(f"\n{'=' * 80}")
                print("ÐYõ¦ TESTE 3: Dados RAW (interval=raw)")
                print("-" * 80)

                request = factory.get(f"/api/telemetry/history/{device_id}/?interval=raw")
                response = view(request, device_id=device_id)
                ...
                if response.status_code == 200:
                    ...
                    if data.get("count", 0) > 0:
                        ...
                        for i, item in enumerate(data.get("data", [])[:5]):
                            print(
                                f"      [{i + 1}] ts={item.get('ts')}, "
                                f"sensor={item.get('sensor_id')}, value={item.get('value')}"
                            )
                    else:
                        print("\n   ƒ?O NENHUM DADO RETORNADO!")
                ...
                print(f"\n{'=' * 80}")
                print("ÐYõ¦ TESTE 4: Query SQL direta no banco")
                print("-" * 80)
                ...
                with connection.cursor() as cursor:
                    ...
                    print(f"   Sensores Ç§nicos: {result[3]}")

                    if result[0] > 0:
                        print("\n   ƒo. HÇ­ dados no banco!")
                        ...
                        print("\n   ÐY\"< Amostra dos dados no banco:")
                        for i, (ts, sensor_id, value) in enumerate(samples):
                            print(f"      [{i + 1}] {ts} | {sensor_id} = {value}")

    print("=" * 80)
    print("ÐYõ¦ TESTE DA API DE TELEMETRIA")
    print("=" * 80)
    test_telemetry_api()
    print("\n" + "=" * 80)
    print("ƒo. TESTES CONCLUÇ?DOS")
    print("=" * 80)
